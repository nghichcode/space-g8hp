...
from ToolsImageCompare_IO import ncio
import ToolsImageCompare_Lib as ncl
...
class NCBox ( wx.Dialog ):
	def __init__(self, tbody, *args, **kw):
		...
		self.hpl = hl.HyperLinkCtrl(self, -1, "ToolsImageCompare", URL="https://nghichcode.com/")
...
class NcImageCheck ( wx.Frame ):
	...
	def __init__(self, *args, **kw):
		...
		# SET config
		try:
			fwpb=open(".\\setting.nc","r")
			global OLD_FOLD
			OLD_FOLD = fwpb.readline().replace('\n','')
			global NEW_FOLD
			NEW_FOLD = fwpb.readline().replace('\n','')
			global OUT_FOLD
			OUT_FOLD = fwpb.readline().replace('\n','')
			fwpb.close()
			print(OLD_FOLD,NEW_FOLD,OUT_FOLD)
		except IOError:
			print("No Config")
		# Style
		...
		self.mitGuide = wx.MenuItem( self.m_menu2, wx.ID_ANY, u"Guide", "\tToolsImageCompare User Guide", wx.ITEM_NORMAL )
		self.mitExit = wx.MenuItem( self.m_menu2, wx.ID_ANY, u"Exit", "\tExit ToolsImageCompare", wx.ITEM_NORMAL )
		...
		self.mitAbout = wx.MenuItem( self.m_menu3, wx.ID_ANY, u"About", "\tInformation About ToolsImageCompare", wx.ITEM_NORMAL )
		...
		# CS4
		gbSizerCS3 = wx.GridBagSizer( 0, 0 )
		gbSizerCS3.SetFlexibleDirection( wx.BOTH )
		gbSizerCS3.SetNonFlexibleGrowMode( wx.FLEX_GROWMODE_SPECIFIED )
		self.m_staticText12 = wx.StaticText( sbSizerChild.GetStaticBox(), wx.ID_ANY, u"An Out Folder", wx.Point( -1,-1 ), wx.DefaultSize, 0 )
		self.m_staticText12.Wrap( -1 )
		self.m_dirPicker012 = wx.DirPickerCtrl( sbSizerChild.GetStaticBox(), wx.ID_ANY, OUT_FOLD, u"Select a folder", wx.DefaultPosition, wx.Size( 350,-1 ), wx.DIRP_DEFAULT_STYLE )
		
		gbSizerCS3.Add( self.m_staticText12, wx.GBPosition( 0, 0 ), wx.GBSpan( 1, 1 ), wx.ALIGN_CENTER_HORIZONTAL|wx.ALIGN_CENTER_VERTICAL|wx.RIGHT|wx.LEFT, 8 )
		gbSizerCS3.Add( self.m_dirPicker012, wx.GBPosition( 0, 1 ), wx.GBSpan( 1, 1 ), wx.ALL, 4 )

		sbSizerChild.Add( gbSizerCS3, 1, wx.EXPAND, 5 )
		# CS5
		...
		self.m_dirPicker012.Bind( wx.EVT_DIRPICKER_CHANGED, self.set_out_folder )
	...
	def set_new_folder( self, event ):
		...
		print(NEW_FOLD)
	def set_out_folder( self, event ):
		global OUT_FOLD
		OUT_FOLD = event.GetPath()
		print(OUT_FOLD)
	
	def ncth(self,cpfdo,cpfdn,cpfou, *kws):
		pcrt,ipsx,ipsy,ipex,ipey = kws
		for rs in ncl.get_data(cpfdo,cpfdn,cpfou, pcrt,ipsx,ipsy,ipex,ipey):
			global STOP_THREAD
	...
	def start_compare( self, event ):
		...
		btn.Disable()
		NC_TH=Thread(target=self.ncth, args=(OLD_FOLD,NEW_FOLD,OUT_FOLD, pcrt,ipsx,ipsy,ipex,ipey))
		NC_TH.start()
...
4. Step 4: Click Execute.
	Result in ".\[parent_of_old_folder]\RESULT_OUT_A2"
	AND .\[parent_of_old_folder]\RESULT_OUT_A2.csv
			""",
			None,
			title="ToolsImageCompare User Guide",
